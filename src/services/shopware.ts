...
export default class shopwareService {
 ...
 public async getProducts(parameter: string, body: Record<string, any>, category: string,) {
    const url = `${this.url}product-listing/${category}?order=${parameter}`;
    const products: Response = await fetch(url, this.createHeader({body: body}))
    const { elements } = await products.json();
    return elements;
  }

  public async searchBy(parameter: string, body: Record<string,any>, value: string) {
